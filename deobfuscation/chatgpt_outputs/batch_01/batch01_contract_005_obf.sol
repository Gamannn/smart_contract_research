...
contract IERC677Callback {
    function receiveApproval(address from, uint256 amount, address token, bytes data) public returns (bool);
}
...
contract Math {
    function absDiff(uint256 v1, uint256 v2) internal constant returns (uint256) {
        return v1 > v2 ? v1 - v2 : v2 - v1;
    }

    function divRound(uint256 v, uint256 d) internal constant returns (uint256) {
        return add(v, d / 2) / d;
    }

    function decimalFraction(uint256 amount, uint256 frac) internal constant returns (uint256) {
        return proportion(amount, frac, 10**18);
    }

    function proportion(uint256 amount, uint256 part, uint256 total) internal constant returns (uint256) {
...
contract StandardToken is IERC20Token, BasicToken, IERC677Token {
...
contract EtherToken is IsContract, AccessControlled, StandardToken, TokenMetadata, Reclaimable {
    ...
    TokenData private _tokenData = TokenData(18, 0);
}