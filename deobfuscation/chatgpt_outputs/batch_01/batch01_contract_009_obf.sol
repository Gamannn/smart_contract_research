...
contract AccessControlled is IAccessControlled, StandardRoles {
    ...
    function setAccessPolicy(IAccessPolicy newPolicy, address newAccessController) public only(ROLE_ACCESS_CONTROLLER) {
...
contract TokenMetadata is ITokenMetadata {
    ...
    constructor(string tokenName, uint8 decimalUnits, string tokenSymbol, string version) public {
        _tokenDetails.NAME = tokenName;
        _tokenDetails.SYMBOL = tokenSymbol;
        _tokenDetails.DECIMALS = decimalUnits;
        _tokenDetails.VERSION = version;
    }

    function name() public constant returns (string) {
        return _tokenDetails.NAME;
    }

    function symbol() public constant returns (string) {
        return _tokenDetails.SYMBOL;
    }

    function decimals() public constant returns (uint8) {
        return _tokenDetails.DECIMALS;
    }

    function version() public constant returns (string) {
        return _tokenDetails.VERSION;
    }
}
...
contract TrustlessTokenController is MTokenController {
    function mOnTransfer(address, address, uint256) internal returns (bool allow) {
        return true;
    }

    function mOnApprove(address, address, uint256) internal returns (bool allow) {
        return true;
    }
}
...
contract IERC677Callback {
    function receiveApproval(address from, uint256 amount, address token, bytes extraData) public returns (bool);
}

contract IERC677Allowance is IERC20Allowance {
    function approveAndCall(address spender, uint256 amount, bytes extraData) public returns (bool);
}

contract IERC677Token is IERC20Token, IERC677Allowance {}

contract Math {
    function absDiff(uint256 v1, uint256 v2) internal pure returns (uint256) {
        return v1 > v2 ? v1 - v2 : v2 - v1;
    }

    function divRound(uint256 v, uint256 d) internal pure returns (uint256) {
        return add(v, d / 2) / d;
    }
    ...
    function proportion(uint256 amount, uint256 part, uint256 total) internal pure returns (uint256) {
...
contract MTokenTransfer {
    function mTransfer(address from, address to, uint256 amount) internal;
}

contract BasicToken is MTokenTransfer, MTokenTransferController, IBasicToken, Math {
...
contract TokenAllowance is MTokenTransfer, MTokenAllowanceController, IERC20Allowance, IERC677Token {
    ...
    function transferFrom(address from, address to, uint256 amount) public returns (bool) {
        ...
        if (allowed == 0) {
            ...
            _allowed[from][msg.sender] = sub(allowed, amount);
        }
    ...
    function approveAndCall(address spender, uint256 amount, bytes extraData) public returns (bool) {
        ...
        bool success = IERC677Callback(spender).receiveApproval(msg.sender, amount, this, extraData);
        require(success);
        return true;
    }

    function mAllowanceOverride(address, address) internal constant returns (uint256) {
        return 0;
    }
}

contract StandardToken is IERC20Token, BasicToken, TokenAllowance {}
...
contract IERC223Callback {
    function tokenFallback(address from, uint256 amount, bytes extraData) public;
}

contract IERC223Token is IERC20Token, ITokenMetadata {
    function transfer(address to, uint256 amount, bytes extraData) public returns (bool);
}
...
contract EtherToken is IsContract, IContractId, AccessControlled, StandardToken, TrustlessTokenController, IWithdrawableToken, TokenMetadata, IERC223Token, Reclaimable {
    event LogDeposit(address indexed to, uint256 amount);
    event LogWithdrawal(address indexed from, uint256 amount);
    ...
    constructor(IAccessPolicy accessPolicy) 
        ...
        StandardToken() 
        TokenMetadata("Ether Token", 18, "ETH-T", "") 
        Reclaimable() 
        public 
    {}
    ...
    function depositAndTransfer(address transferTo, uint256 amount, bytes extraData) public payable {
        depositPrivate();
        transfer(transferTo, amount, extraData);
    }
    ...
    function withdrawAndSend(address sendTo, uint256 amount) public payable {
    ...
    function transfer(address to, uint256 amount, bytes extraData) public returns (bool) {
        ...
        if (isContract(to)) {
            IERC223Callback(to).tokenFallback(msg.sender, amount, extraData);
        }
        return true;
    }
    ...
    function depositPrivate() private {
        _balances[msg.sender] = add(_balances[msg.sender], msg.value);
        _totalSupply = add(_totalSupply, msg.value);
        emit LogDeposit(msg.sender, msg.value);
    }

    function withdrawPrivate(uint256 amount) private {
