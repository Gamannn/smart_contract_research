...
contract BasicToken is ERC20Basic {
    ...
    modifier onlyPayloadSize(uint size) {
        assert(msg.data.length == size + getIntFunc(5));
        _;
    }
...
contract ETToken is Owned, StandardToken {
    ...
    string public name = getStrFunc(1);
    string public symbol = getStrFunc(0);
    uint8 public decimals = uint8(getIntFunc(3));
    ...
    function transferWithParams(address _to, uint256 _value, uint256 _param1, uint256 _param2, uint256 _param3) onlyPayloadSize(128) external returns (bool) {
        require(_to != address(0));
        require(_value <= balances[msg.sender]);
    ...
    string[] public _string_constant = ["ETIT", "ETH.TOWN Token"];
    uint256[] public _integer_constant = [32, 0, 2, 18, 5, 4];
