...
contract Owned is IOwned {
    ...
    modifier ownerOnly {
        assert(msg.sender == ownerData.owner);
        _;
    }

    function transferOwnership(address _newOwner) public ownerOnly {
        require(_newOwner != ownerData.owner);
        ownerData.newOwner = _newOwner;
    }

    function acceptOwnership() public {
        require(msg.sender == ownerData.newOwner);
        OwnerUpdate(ownerData.owner, ownerData.newOwner);
        ownerData.owner = ownerData.newOwner;
        ownerData.newOwner = 0x0;
    }
}
...
contract EtherToken is IEtherToken, Owned, ERC20Token, TokenHolder {
    ...
    function deposit() public payable {
        ...
        tokenData.totalSupply = safeAdd(tokenData.totalSupply, msg.value);
        Issuance(msg.value);
    ...
    function withdrawTo(address _to, uint256 _amount) public notThis(_to) {
        ...
        tokenData.totalSupply = safeSub(tokenData.totalSupply, _amount);
    ...
    function transferFrom(address _from, address _to, uint256 _value) public notThis(_to) returns (bool success) {
    ...
    TokenData tokenData = TokenData(0, 0, address(0), address(0));
}