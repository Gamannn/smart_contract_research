...
contract HegicETHPool is ILiquidityPool, Ownable, ERC20("Hegic ETH LP Token", "writeETH") {
    ...
    function withdraw(uint amount, uint maxBurn) public returns (uint burnAmount) {
        burnAmount = withdraw(amount);
        require(burnAmount <= maxBurn, "Pool: Burn limit is too small");
    }

    function withdraw(uint amount) public returns (uint burnAmount) {
        require(amount <= availableBalance(), "Pool: Insufficient unlocked funds");
        burnAmount = amount.mul(totalSupply()).div(totalBalance());
        require(burnAmount <= balanceOf(msg.sender), "Pool: Amount is too large");
        require(burnAmount > 0, "Pool: Amount is too small");
    ...
    function lock(uint amount) public override onlyOwner {
        require(lockedAmount.add(amount).mul(10).div(totalBalance()) < 8, "Pool: Insufficient unlocked funds");
...
abstract contract HegicOptions is Ownable, SpreadLock {
    ...
    uint256 private impliedVolRate;
    uint256 private maxSpread;
}

contract HegicCallOptions is HegicOptions {
    constructor(IERC20 DAI, AggregatorInterface pp, IUniswapFactory ex) HegicOptions(DAI, pp, ex, HegicOptions.OptionType.Call) public {
    ...
    function exchange() public returns (uint exchangedAmount) {
    ...
    function exercise(uint optionID) public {
        ...
        require(token.transferFrom(option.holder, address(this), option.strikeAmount), "Insufficient funds");
        uint amount = exchange();
        pool.send(option.holder, option.amount);
        emit Exercise(optionID, amount);
    }

    uint256 private priceDecimals = 1e8;
    uint256 private activationTime = 15 minutes;
    bool private highSpreadLockEnabled;
}

contract HegicPutOptions is HegicOptions {
    constructor(IERC20 DAI, AggregatorInterface pp, IUniswapFactory ex) HegicOptions(DAI, pp, ex, HegicOptions.OptionType.Put) public {
        pool = new HegicERCPool(DAI);
    }

    function exchange() public returns (uint) {
    ...
    function exchange(uint amount) public returns (uint exchangedAmount) {
        ...
        if (exShare > maxSpread.mul(uint(priceProvider.latestAnswer())).div(1e8)) {
            highSpreadLockEnabled = false;
