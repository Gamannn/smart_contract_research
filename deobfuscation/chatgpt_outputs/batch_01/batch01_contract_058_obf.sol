...
contract GiftToken is BurnableToken, Pausable {
    ...
    function GiftToken(address _ico) public {
        pause();
        setIcoAddress(_ico);
        totalSupply_ = 1e8 * (10 ** uint256(decimals));
        balances[_ico] = totalSupply_;
        Transfer(address(0), _ico, totalSupply_);
    }

    function setIcoAddress(address _ico) public onlyOwner {
        require(_ico != address(0));
        require(balanceOf(icoAddress) == 0);
        icoAddress = _ico;
...
contract GiftCrowdsale is Pausable {
    ...
    uint256 public minimumInvestment = 25e16;
    ...
    function discount() public view returns (uint256) {
        if (now > startTimestamp + 2 weeks) return 0;
        if (now > startTimestamp + 1 weeks) return 5;
        if (now > startTimestamp + 1 days) return 15;
        return 25;
    }
    ...
    function sellTokens() public payable whenSaleIsOpen whenWhitelisted(msg.sender) whenNotPaused {
        ...
        uint256 tokensAmount = msg.value.mul(exchangeRate).mul(100 + _bonus).div(100 - _discount);
