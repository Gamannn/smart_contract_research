...
contract Sparkle is ERC20Detailed {
    ...
    event SparkleRedistribution(address indexed from, uint amount);
    event Mint(address indexed to, uint amount);
    event Sell(address indexed from, uint amount);

    uint256 public _totalSupply;
    uint256 public _tobinsCollected;
    address payable public creator;
    uint256 private COST_PER_TOKEN;
    uint256 private TAX;
    ...
    function sellSparkle(uint256 amount) public returns (bool) {
        ...
        uint256 reward = amount.mul(COST_PER_TOKEN);
