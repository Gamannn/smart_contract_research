...
contract FlightDelayConstants {
    event LogPolicyApplied(uint _policyId, address _customer, bytes32 strCarrierFlightNumber, uint ethPremium);
    event LogPolicyAccepted(uint _policyId, uint _statistics0, uint _statistics1, uint _statistics2, uint _statistics3, uint _statistics4, uint _statistics5);
    event LogPolicyPaidOut(uint _policyId, uint ethAmount);
    event LogPolicyExpired(uint _policyId);
    event LogPolicyDeclined(uint _policyId, bytes32 strReason);
    event LogPolicyManualPayout(uint _policyId, bytes32 strReason);
    event LogSendFunds(address _recipient, uint8 _from, uint ethAmount);
    event LogReceiveFunds(address _sender, uint8 _to, uint ethAmount);
    event LogSendFail(uint _policyId, bytes32 strReason);
    event LogOraclizeCall(uint _policyId, bytes32 hexQueryId, string _oraclizeUrl, uint256 _oraclizeTime);
    event LogOraclizeCallback(uint _policyId, bytes32 hexQueryId, string _result, bytes hexProof);
    event LogSetState(uint _policyId, uint8 _policyState, uint _stateTime, bytes32 _stateMessage);
    event LogExternal(uint256 _policyId, address _address, bytes32 _externalId);
...
contract FlightDelayDatabaseInterface is FlightDelayDatabaseModel {
    ...
    function setState(uint _policyId, policyState _state, uint _stateTime, bytes32 _stateMessage) public;
    ...
    function createUpdateRisk(bytes32 _carrierFlightNumber, bytes32 _departureYearMonthDay, uint _arrivalTime) public returns (bytes32 _riskId);
    ...
    function createOraclizeCallback(bytes32 _queryId, uint _policyId, oraclizeState _oraclizeState, uint _oraclizeTime) public;
...
contract FlightDelayLedger is FlightDelayControlledContract, FlightDelayLedgerInterface, FlightDelayConstants {
    ...
    struct scalar2Vector {
        uint256 MAX_DEPARTURE_LIM;
        uint256 MIN_DEPARTURE_LIM;
        address controller;
        uint256 ORACLIZE_GASPRICE;
        uint256 ORACLIZE_GAS;
        uint256 CONTRACT_DEAD_LINE;
        uint256 MAX_FLIGHT_DURATION;
        uint256 CHECK_PAYOUT_OFFSET;
        uint256 MIN_TIME_BEFORE_DEPARTURE;
        uint8 RESERVE_PERCENT;
        uint8 REWARD_PERCENT;
        uint256 MAX_CUMULATED_WEIGHTED_PREMIUM;
        uint256 MAX_PAYOUT_GBP;
        uint256 MAX_PREMIUM_GBP;
        uint256 MIN_PREMIUM_GBP;
        uint256 MAX_PAYOUT_USD;
        uint256 MAX_PREMIUM_USD;
        uint256 MIN_PREMIUM_USD;
        uint256 MAX_PAYOUT_EUR;
        uint256 MAX_PREMIUM_EUR;
        uint256 MIN_PREMIUM_EUR;
        uint256 MAX_PAYOUT;
        uint256 MAX_PREMIUM;
        uint256 MIN_PREMIUM;
        uint256 MIN_OBSERVATIONS;
    }

    scalar2Vector s2c = scalar2Vector(
        0, 0, address(0), 4000000000, 700000, 1922396399, 2 days, 15 minutes, 24 hours, 1, 2, 60 ether, 270 wei, 25000 wei, 1300 wei, 35000 wei, 34000 wei, 1700 wei, 30000 wei, 29000 wei, 1500 wei, 1100 finney, 1 ether, 50 finney, 10
    );
}