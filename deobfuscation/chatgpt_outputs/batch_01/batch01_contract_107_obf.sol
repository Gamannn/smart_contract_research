...
contract Restricted is Ownable {
    event MonethaAddressSet(address indexed addr, bool isMonethaAddress);
    ...
    function setMonethaAddress(address addr, bool isMonethaAddress) public onlyOwner {
        isMonethaAddress[addr] = isMonethaAddress;
        emit MonethaAddressSet(addr, isMonethaAddress);
    }
}
...
contract CanReclaimEther is Ownable {
    ...
    function reclaimEtherTo(address to, uint256 value) external onlyOwner {
        require(to != address(0), "zero address is not allowed");
        to.transfer(value);
        emit ReclaimEther(to, value);
    }
}
...
contract CanReclaimTokens is Ownable {
    ...
    function reclaimTokenTo(ERC20Basic token, address to, uint256 value) external onlyOwner {
        require(to != address(0), "zero address is not allowed");
        token.safeTransfer(to, value);
        emit ReclaimTokens(to, value);
    }
}
...
contract MonethaVoucher is IMonethaVoucher, Restricted, Pausable, IERC20, CanReclaimEther, CanReclaimTokens {
    ...
    uint256 constant RATE_COEFFICIENT = 1e18;
    uint256 constant HALF_YEAR_IN_SECONDS_AVG = 15778463; // Average half-year in seconds
    uint256 constant YEAR_IN_SECONDS_AVG = 31556926; // Average year in seconds
    ...
    function _updateVoucherMthEthRate() internal {
        voucherMthEthRate = voucherMthRate.mul(mthEthRate).div(RATE_COEFFICIENT);
    }
    ...
    function _vouchersToWei(uint256 value) internal view returns (uint256) {
        return value.mul(RATE_COEFFICIENT).div(voucherMthEthRate);
    }

    function _weiToVouchers(uint256 value) internal view returns (uint256) {
        return value.mul(voucherMthEthRate).div(RATE_COEFFICIENT);
    }
    ...
    function _distributedTo(address owner, uint16 currentHalfYear) internal view returns (uint256) {
        ...
        if (currentHalfYear > 0) {
            balance = balance.add(distributed[currentHalfYear - 1][owner]);
        }
        return balance;
    }
