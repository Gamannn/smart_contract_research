...
contract ActivityStorage is StorageBase {
    ...
    function pauseActivity(uint16 _activityId) external onlyOwner {
        activities[_activityId].isPaused = true;
    }

    function unpauseActivity(uint16 _activityId) external onlyOwner {
        activities[_activityId].isPaused = false;
    }
...
contract ActivityCore is LogicBase {
    ...
    function ActivityCore(address _nftAddress, address _storageAddress) LogicBase(_nftAddress, _storageAddress) public {
    ...
    function deleteActivity(uint16 _activityId) external onlyOwner whenPaused {
        activityStorage.deleteActivity(_activityId);
    }

    function getActivity(uint16 _activityId) external view returns (
        bool isPaused,
        uint16 buyLimit,
        uint128 packPrice,
        uint64 startDate,
    ...
    function bid(uint16 _activityId, uint16 _packId) external payable whenNotPaused {
        bool isPaused;
        uint16 buyLimit;
        uint128 packPrice;
        uint64 startDate;
        uint64 endDate;
        (isPaused, buyLimit, packPrice, startDate, endDate) = activityStorage.activities(_activityId);
        require(!isPaused);
        require(buyLimit > 0);
