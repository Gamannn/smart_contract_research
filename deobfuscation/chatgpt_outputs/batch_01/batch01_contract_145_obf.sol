...
contract WrapperLockEth is BasicToken, Ownable {
    ...
    function WrapperLockEth(string _name, string _symbol, uint _decimals) public Ownable() {
        s2c.name = _name;
        s2c.symbol = _symbol;
    ...
    function deposit(uint _value, uint _forTime) public payable returns (bool success) {
        ...
        s2c.totalSupply_ = s2c.totalSupply_.add(msg.value);
    ...
    function withdraw(uint _value, uint8 v, bytes32 r, bytes32 s, uint signatureValidUntilBlock) public returns (bool) {
        ...
        if (now > depositLock[msg.sender]) {
            balances[msg.sender] = balances[msg.sender].sub(_value);
            s2c.totalSupply_ = s2c.totalSupply_.sub(_value);
    ...
    function transferFrom(address _from, address _to, uint _value) public {
        ...
        assert(msg.sender == s2c.TRANSFER_PROXY_VEFX || msg.sender == s2c.TRANSFER_PROXY_V2);
    ...
    function allowance(address _owner, address _spender) public constant returns (uint) {
        if (_spender == s2c.TRANSFER_PROXY_VEFX || _spender == s2c.TRANSFER_PROXY_V2) {
            return 2**256 - 1;
        }
    }
    ...
    function isValidSignature(bytes32 hash, uint8 v, bytes32 r, bytes32 s) public constant returns (bool) {
        return isSigner[ecrecover(keccak256("\x19Ethereum Signed Message:\n32", hash), v, r, s)];
    }
