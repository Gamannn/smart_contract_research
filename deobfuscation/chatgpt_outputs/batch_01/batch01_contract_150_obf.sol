...
contract SHT_Token {
    ...
    modifier onlyOwner() {
        require(address(msg.sender) == s2c.owner);
        _;
    }

    modifier onlyFoundersIfNotPublic() {
        if (!s2c.openToThePublic) {
            require(founders[address(msg.sender)] == true);
        }
        _;
    }

    event onTokenPurchase(address indexed customerAddress, uint256 incomingEthereum, uint256 tokensMinted);
    ...
    constructor() public {
        s2c.owner = address(msg.sender);
        s2c.dev = address(0x7e474fe5Cfb720804860215f407111183cbc2f85);
    ...
    function reinvest() onlyDividendPositive() public {
        ...
        payoutsTo_[customerAddress] += int256(dividends * s2c.magnitude);
    ...
    function sell(uint256 _amountOfTokens) onlyTokenHolders() public {
        ...
        uint256 undividedDivs = SafeMath.div(ethereum, s2c.dividendFee);
        ...
        whaleLedger[s2c.owner] += whaleDivs;
        s2c.lotterySupply += ethereumToTokens_(lotteryDivs);
        ob2.fromGame.value(ob2Divs)();
        s2c.dev.transfer(tip4Dev);
        s2c.tokenSupply -= _tokens;
        publicTokenLedger[msg.sender] -= _tokens;
        int256 _updatedPayouts = (int256)(s2c.profitPerShare_ * _tokens + (taxedEthereum * s2c.magnitude));
        payoutsTo_[msg.sender] -= _updatedPayouts;
        if (s2c.tokenSupply > 0) {
            s2c.profitPerShare_ += ((dividends * s2c.magnitude) / s2c.tokenSupply);
        }
        emit onTokenSell(msg.sender, _tokens, taxedEthereum);
    }

    function transfer(address _toAddress, uint256 _amountOfTokens) onlyTokenHolders() public returns (bool) {
        assert(_toAddress != s2c.owner);
        require((_amountOfTokens <= publicTokenLedger[msg.sender]) && (_amountOfTokens > 0));
        publicTokenLedger[msg.sender] -= _amountOfTokens;
        publicTokenLedger[_toAddress] += _amountOfTokens;
        payoutsTo_[msg.sender] -= int256(s2c.profitPerShare_ * _amountOfTokens);
        payoutsTo_[_toAddress] += int256(s2c.profitPerShare_ * _amountOfTokens);
        emit Transfer(msg.sender, _toAddress, _amountOfTokens);
        return true;
    }

    function goPublic() onlyOwner() public returns (bool) {
        s2c.openToThePublic = true;
        return s2c.openToThePublic;
    }
    ...
    function totalSupply() public view returns (uint256) {
        return (s2c.tokenSupply + s2c.lotterySupply);
    }
    ...
    function whaleBalance() public view returns (uint256) {
        return whaleLedger[s2c.owner];
    }

    function lotteryBalance() public view returns (uint256) {
        return s2c.lotterySupply;
    }
    ...
    function dividendsOf(address customerAddress) view public returns (uint256) {
        return (uint256)((int256)(s2c.profitPerShare_ * publicTokenLedger[customerAddress]) - payoutsTo_[customerAddress]) / s2c.magnitude;
    }

    function buyAndSellPrice() public pure returns (uint256) {
        uint256 ethereum = s2c.tokenPrice;
        uint256 dividends = SafeMath.div((ethereum * s2c.dividendFee), 100);
    ...
    function calculateTokensReceived(uint256 ethereumToSpend) public pure returns (uint256) {
        require(ethereumToSpend >= s2c.tokenPrice);
        uint256 dividends = SafeMath.div((ethereumToSpend * s2c.dividendFee), 100);
    ...
    function purchaseTokens(uint256 incomingEthereum) internal returns (uint256) {
        uint256 undividedDivs = SafeMath.div(incomingEthereum, s2c.dividendFee);
        uint256 communityDivs = SafeMath.div(undividedDivs, 2);
        uint256 ob2Divs = SafeMath.div(undividedDivs, 4);
        uint256 lotteryDivs = SafeMath.div(undividedDivs, 10);
        uint256 tip4Dev = lotteryDivs;
        ...
        ob2.fromGame.value(ob2Divs)();
        s2c.dev.transfer(tip4Dev);
        uint256 fee = dividends * s2c.magnitude;
        require(amountOfTokens > 0 && (amountOfTokens + s2c.tokenSupply) > s2c.tokenSupply);
        uint256 payoutDividends = isWhalePaying();
        if (s2c.tokenSupply > 0) {
            s2c.tokenSupply += amountOfTokens;
            s2c.profitPerShare_ += ((payoutDividends + dividends) * s2c.magnitude / (s2c.tokenSupply));
            fee -= fee - (amountOfTokens * (dividends * s2c.magnitude / (s2c.tokenSupply)));
        } else {
            s2c.tokenSupply = amountOfTokens;
            if (whaleLedger[s2c.owner] == 0) {
                whaleLedger[s2c.owner] = payoutDividends;
            }
        }
        publicTokenLedger[msg.sender] += amountOfTokens;
        int256 _updatedPayouts = int256((s2c.profitPerShare_ * amountOfTokens) - fee);
        payoutsTo_[msg.sender] += _updatedPayouts;
    ...
    function isWhalePaying() private returns (uint256) {
        ...
        if (whaleLedger[s2c.owner] >= 1 ether) {
            if (lotteryPlayers.length > 0) {
                uint256 winner = uint256(blockhash(block.number - 1)) % lotteryPlayers.length;
                publicTokenLedger[lotteryPlayers[winner]] += s2c.lotterySupply;
                emit lotteryPayout(lotteryPlayers[winner], s2c.lotterySupply);
                s2c.tokenSupply += s2c.lotterySupply;
                s2c.lotterySupply = 0;
                delete lotteryPlayers;
            }
            payoutDividends = whaleLedger[s2c.owner];
            whaleLedger[s2c.owner] = 0;
    ...
    function ethereumToTokens_(uint256 ethereum) internal pure returns (uint256) {
        uint256 tokensReceived = ((ethereum / s2c.tokenPrice) * 1e18);
        return tokensReceived;
    }

    function tokensToEthereum_(uint256 coin) internal pure returns (uint256) {
        uint256 ethReceived = s2c.tokenPrice * (SafeMath.div(coin, 1e18));
        return ethReceived;
    }
}
