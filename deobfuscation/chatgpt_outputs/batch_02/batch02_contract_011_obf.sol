...
contract Exchange is SafeMath {
    ...
    function Exchange(address feeAccount_, address accountLevelsAddr_, uint feeMake_, uint feeTake_, uint feeRebate_) {
    ...
    function changeAccountLevelsAddr(address accountLevelsAddr_) {
        if (msg.sender != feeAccount) throw;
        accountLevelsAddr = accountLevelsAddr_;
    }
    ...
    function changeFeeMake(uint feeMake_) {
        if (msg.sender != feeAccount) throw;
        if (feeMake_ > 10 finney) throw;
        if (feeMake_ < 1 finney) throw;
        feeMake = feeMake_;
    }

    function changeFeeTake(uint feeTake_) {
        if (msg.sender != feeAccount) throw;
        if (feeTake_ > 10 finney) throw;
        if (feeTake_ < 1 finney) throw;
        feeTake = feeTake_;
    }

    function changeFeeRebate(uint feeRebate_) {
        if (msg.sender != feeAccount) throw;
        if (feeRebate_ > 10 finney) throw;
        if (feeRebate_ < 1 finney) throw;
        feeRebate = feeRebate_;
    }
    ...
    function tradeBalances(address tokenGet, uint amountGet, address tokenGive, uint amountGive, address user, uint amount) private {
        ...
        tokens[tokenGet][user] = safeAdd(tokens[tokenGet][user], safeSub(safeSub(amount, feeMakeXfer), feeRebateXfer));
