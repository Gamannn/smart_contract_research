...
contract LittleSisterToken is ERC20Token {
    ...
    function sell(uint tokens) public returns (uint256 revenue) {
        require(sellPrice() > 0);
        require(!lost);
        ...
        Transfer(msg.sender, this, tokens);
        return revenue;
    }
...
contract BigSisterToken is LittleSisterToken {
    ...
    function BigSisterToken(uint256 _maxSupply, uint256 _buyPrice, uint256 _sellPrice, uint8 _vig, uint _buyingHorizon, uint _maximumLifetime) {
...
contract PriceIncreasingLittleSisterToken is LittleSisterToken {
    ...
    function buy() public payable returns (uint tokens) {
        ...
        tokens = super.buy();
        sisterContract.sisterCheckPrice(tokens);
        return tokens;
    }
}
...
contract PriceIncreasingToken is BigSisterToken {
    ...
    function PriceIncreasingToken(uint256 _maxSupply, uint256 _buyPrice, uint256 sellPrice_, uint8 _vig, uint256 _thresholdAmount, uint256 _priceIncrease, uint _buyingHorizon, uint _maximumLifetime) 
    ...
    function sisterCheckPrice(uint256 tokens) public {
        require(msg.sender == sister);
        _checkPrice(tokens);
    }

    function _checkPrice(uint256 tokens) internal {
        currentBatch += tokens;
...
contract TimedEvidenceToken is PriceIncreasingToken {
    ...
    function TimedEvidenceToken(uint256 _maxSupply, uint256 _buyPrice, uint8 _vig, uint256 _thresholdAmount, uint256 _priceIncrease, uint _evidenceInterval, uint8 _requiredEvidence, uint _dateSisterWins, uint _buyingHorizon, uint _maximumLifetime) 
        PriceIncreasingToken(_maxSupply, _buyPrice, 0, _vig, _thresholdAmount, _priceIncrease, _buyingHorizon, _maximumLifetime) {
...
contract TrumpImpeachmentToken is TrumpOracle, TimedEvidenceToken, StringOps {
    ...
    function TrumpImpeachmentToken() 
        TimedEvidenceToken(2000000 * unit, 69 finney / unit, 40, 6600 * unit, 100 szabo / unit, 2 days, 3, 1611014400, 222 days, 7 years) {
        lastEvidence = "N/A";
