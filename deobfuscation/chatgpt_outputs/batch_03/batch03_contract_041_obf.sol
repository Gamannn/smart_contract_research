...
contract AuctionContract {
    event Bid(address bidder, uint endTime, uint timeLeft, uint bidAmount);
    ...
    function placeBid() payable public {
        ...
        if (auction.timeLeft == 0 || auction.endTime <= now) {
            auction.lastBidder = auction.currentBidder;
            auction.timeLeft = 2 hours;
            auction.endTime = now + auction.timeLeft;
            auction.currentBidder = msg.sender;
            ...
            auction.lastBidder.transfer(((this.balance - 5000000000000000) / 10) * 8);
        }
    }
