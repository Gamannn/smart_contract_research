...
contract PresaleToken {
    string public constant name = "SQEX";
    uint public constant decimals = 18;
    uint public constant PRICE = 1700;
    uint public constant TOKEN_SUPPLY_LIMIT = PRICE * 2500;
    
    address public escrow;
    address public tokenManager;
    address public crowdsaleManager;
    
    enum State { Init, Running, Paused, Migrating, Migrated }
    ...
    modifier onlyTokenManager() {
        if (msg.sender != tokenManager) throw;
        _;
    }
    
    modifier onlyCrowdsaleManager() {
        if (msg.sender != crowdsaleManager) throw;
        _;
    }
    ...
    event LogBuy(address indexed buyer, uint amount);
    event LogBurn(address indexed owner, uint amount);
    ...
    function setPresaleState(State _nextState) public onlyTokenManager {
        bool canSwitchState = 
            ...
            ((currentState == State.Running || currentState == State.Paused) && _nextState == State.Migrating && crowdsaleManager != 0x0) ||
            (currentState == State.Paused && _nextState == State.Running) ||
            (currentState == State.Migrating && _nextState == State.Migrated && totalSupply == 0);
