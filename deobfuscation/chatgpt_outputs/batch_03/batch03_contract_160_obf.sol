...
contract HydroSwap {
    ...
    address public tokenProxyAddress;
    ...
    function swap(
        ...
        bytes32 r,
        bytes32 s
    ) external payable returns (uint256 takerTokenFilledAmount) {
        ...
        if (takerTokenAddress == wethAddress) {
            require(takerTokenAmount == msg.value, "WETH_AMOUNT_MISMATCH");
        ...
        takerTokenFilledAmount = Exchange(exchangeAddress).fillOrder(orderAddresses, orderValues, takerTokenAmount, true, v, r, s);
        require(takerTokenFilledAmount == takerTokenAmount, "FILL_ORDER_ERROR");
