...
contract DexBrokerage is Ownable {
    ...
    function approveCurrencyToken(address currencyTokenAddress, bool isApproved) onlyAdmin public {
    ...
    function trade(uint256[11] tradeValues, address[4] tradeAddresses, uint8[2] v, bytes32[4] rs) onlyAdmin public returns (bool) {
        ...
        bytes32 orderHash = keccak256(abi.encodePacked(address(this), tradeAddresses[0], tradeValues[7], tradeAddresses[1], tradeValues[8], tradeValues[9], tradeValues[2], tradeAddresses[2]));
        bytes32 tradeHash = keccak256(abi.encodePacked(address(this), tradeAddresses[1], tradeValues[3], tradeAddresses[0], tradeValues[4], tradeValues[5], tradeValues[6], tradeAddresses[3]));
