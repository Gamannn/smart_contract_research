...
contract Owned {
    ...
    function setOwner(address _address) public onlyAdmin {
        owner = _address;
    }
    ...
    function addModerator(address _address) public onlyAdmin {
...
contract BankWallet is Pausable, RequiringAuthorization, SafeMath {
    ...
    event Fund(address indexed account, uint amount);
    ...
    function withdraw(address _token, uint _amount) public onlyAuthorized returns (bool _success) {
        _success = false;
        if (_token == address(0)) {
            ...
            if (tokenAmount > token.balanceOf(this)) {
                return false;
            }
            _success = token.transfer(receiver, tokenAmount);
        }
        if (_success) {
    ...
    function deposit(address _address, uint _amount, bool _chargeGas) public onlyAuthorized {
        ...
        casino.deposit(_address, _amount, _chargeGas);
    ...
    function _approveForCasino(address _address, uint _amount) internal returns (bool _success) {
        ...
        _success = token.approve(_address, safeMul(_amount, oneEdg));
    }
}