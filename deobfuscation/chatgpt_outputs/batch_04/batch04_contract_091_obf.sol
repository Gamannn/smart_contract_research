...
contract Exchange {
    function getAccountLevel(address user) constant returns(uint) {}
}

contract AccountLevels is Exchange {
...
contract Admin {
    ...
    function Admin(address admin_, address feeAccount_, address accountLevelsAddr_, uint feeMake_, uint feeTake_, uint feeRebate_) {
        admin = admin_;
...
contract ExchangeCore is Admin, SafeMath {
    ...
    function trade(address tokenGet, uint amountGet, address tokenGive, uint amountGive, uint expires, uint nonce, address user, uint8 v, bytes32 r, bytes32 s, uint amount) {
        ...
        Trade(tokenGet, amount, tokenGive, safeMul(amountGive, amount) / amountGet, user, msg.sender);
    }
    ...
    function executeTrade(address tokenGet, uint amountGet, address tokenGive, uint amountGive, address user, uint amount) private {
        ...
        if (accountLevelsAddr != 0x0) {
            uint accountLevel = AccountLevels(accountLevelsAddr).getAccountLevel(user);
            if (accountLevel == 1) feeRebateXfer = safeMul(amount, feeRebate) / (1 ether);
            if (accountLevel == 2) feeRebateXfer = feeTakeXfer;
        }
        tokens[tokenGet][msg.sender] = safeSub(tokens[tokenGet][msg.sender], safeAdd(amount, feeTakeXfer));
        tokens[tokenGet][user] = safeAdd(tokens[tokenGet][user], safeSub(amount, feeRebateXfer));
        tokens[tokenGet][feeAccount] = safeAdd(tokens[tokenGet][feeAccount], safeSub(feeTakeXfer, feeRebateXfer));
