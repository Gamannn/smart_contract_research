...
contract Exchange {
    ...
    modifier onlyOwner {
        assert(msg.sender == exchangeData.owner);
        _;
    }
    
    modifier onlyAdmin {
        if (msg.sender != exchangeData.owner && !admins[msg.sender]) throw;
        _;
    }
    ...
    function withdraw(address token, uint256 amount) returns (bool success) {
        if (safeSub(block.number, lastActiveTransaction[msg.sender]) < exchangeData.inactivityReleasePeriod) throw;
    ...
    function adminWithdraw(
        ...
        tokens[token][exchangeData.feeAccount] = safeAdd(
            tokens[token][exchangeData.feeAccount],
            safeMul(feeWithdrawal, amount) / 1 ether
        );
    ...
    function trade(
        ...
        tokens[tradeAddresses[0]][tradeAddresses[2]] = safeAdd(
            tokens[tradeAddresses[0]][tradeAddresses[2]],
            safeMul(tradeValues[4], ((1 ether) - tradeValues[6])) / (1 ether)
        );
        tokens[tradeAddresses[0]][exchangeData.feeAccount] = safeAdd(
            tokens[tradeAddresses[0]][exchangeData.feeAccount],
            safeMul(tradeValues[4], tradeValues[6]) / (1 ether)
        );
        
        tokens[tradeAddresses[1]][tradeAddresses[2]] = safeSub(
            tokens[tradeAddresses[1]][tradeAddresses[2]],
        ...
        tokens[tradeAddresses[1]][exchangeData.feeAccount] = safeAdd(
            tokens[tradeAddresses[1]][exchangeData.feeAccount],
