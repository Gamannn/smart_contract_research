...
contract BasicToken is ERC20Basic {
    ...
    function totalSupply() public view returns (uint256) {
        return storageStruct.totalSupply_;
    }
...
contract Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        OwnershipTransferred(storageStruct.owner, newOwner);
        storageStruct.owner = newOwner;
    }
}
...
contract WrapperLockEth is BasicToken, Ownable {
    ...
    function WrapperLockEth(string _name, string _symbol, uint _decimals) Ownable() {
        storageStruct.name = _name;
        storageStruct.symbol = _symbol;
        storageStruct.decimals = _decimals;
    ...
    function deposit(uint _value, uint _forTime) public payable returns (bool success) {
        ...
        storageStruct.totalSupply_ = storageStruct.totalSupply_.add(msg.value);
    ...
    function withdraw(
        ...
        if (now > depositLock[msg.sender]) {
            balances[msg.sender] = balances[msg.sender].sub(_value);
            storageStruct.totalSupply_ = storageStruct.totalSupply_.sub(msg.value);
    ...
    function transferFrom(address _from, address _to, uint _value) public {
        ...
        assert(msg.sender == storageStruct.TRANSFER_PROXY_V2 || msg.sender == storageStruct.TRANSFER_PROXY_VEFX);
        balances[_to] = balances[_to].add(_value);
    ...
    function allowance(address _owner, address _spender) public constant returns (uint) {
        if (_spender == storageStruct.TRANSFER_PROXY_VEFX || _spender == storageStruct.TRANSFER_PROXY_V2) {
            return 2**256 - 1;
        }
    }
    ...
    StorageStruct storageStruct = StorageStruct(
        0x00,
        0,
        "",
        "",
