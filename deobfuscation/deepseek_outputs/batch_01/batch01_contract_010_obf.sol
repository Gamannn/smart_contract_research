...
contract AbstractPaymentEscrow is Ownable {
    ...
    event Payment(address indexed _customer, uint indexed _projectId, uint _amount);
    event Withdraw(address indexed _wallet, uint _amount);
...
contract SingularDTVFund {
    ...
    function withdrawReward() public returns (uint) {
        uint amount = calcReward(msg.sender) + owed[msg.sender];
    ...
    function softWithdrawRewardFor(address forAddress) external returns (uint) {
        uint amount = calcReward(forAddress);
        rewardAtTimeOfWithdraw[forAddress] = totalReward;
        owed[forAddress] += amount;
        return amount;
    }
...
contract TokitDeployer is Ownable {
    ...
    function deployToken(
        ...
        var (token,,) = registry.lookup(_customer, _projectId);
