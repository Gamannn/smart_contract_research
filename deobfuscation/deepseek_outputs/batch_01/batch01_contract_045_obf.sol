...
contract HDX20 {
    ...
    function buyTokenFromGame(address _customerAddress, address _referrer_address) 
        public payable onlyFromGameWhiteListed returns(uint256) 
    {
        ...
        if (_referrer_address != address(0) && _referrer_address != _customerAddress) {
            uint256 _ethReferrer = _fee.mul(contractData.referrerFee + superReferrerRate[_referrer_address]).div(100);
    ...
    function payWithToken(uint256 _eth, address _player_address) 
        public onlyFromGameWhiteListed returns(uint256) 
    {
    ...
    function moveAccountOut() public onlyTokenHolders {
        ...
        uint256 balance = tokenBalanceLedger[_customerAddress];
        uint256 _eth = balance.mul(contractData.tokenPrice).div(contractData.magnitude);
    ...
    function ethBalanceOf(address _customerAddress) view public returns(uint256) {
        ...
        uint256 value = _balance.mul(_price).div(contractData.magnitude);
        return value;
    }
