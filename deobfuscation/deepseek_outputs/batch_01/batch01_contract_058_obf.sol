...
contract GiftCrowdsale is Pausable {
    ...
    uint256 public constant minimumInvestment = 25e16; // 0.25 ETH
    ...
    function discount() public view returns (uint256) {
        if (now > endThirdPeriodTimestamp) return 0;
        if (now > endSecondPeriodTimestamp) return 5;
        if (now > endFirstPeriodTimestamp) return 15;
        return 25;
    }
