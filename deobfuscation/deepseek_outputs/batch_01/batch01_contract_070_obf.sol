...
contract BullToken is BurnableToken, PausableToken {
    ...
    function BullToken() public {
        totalSupply = 55000000 * 10 ** uint256(18);
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool) {
        require(transfersEnabled || _from == owner);
...
contract Whitelist is CurationRights {
    mapping (address => bool) public whitelist;

    function Whitelist() public {
    }

    function addToWhitelist(address _address) external onlyCurator {
        require(_address != 0x0 && !whitelist[_address]);
        whitelist[_address] = true;
    }

    function removeFromWhitelist(address _address) external onlyCurator {
        require(_address != 0x0 && whitelist[_address]);
        whitelist[_address] = false;
    }

    function isWhitelisted(address _address) constant external returns (bool) {
...
contract BullCrowdsale is CappedCrowdsale, RefundableCrowdsale {
    using SafeMath for uint256;

    Whitelist public whitelist;

    function BullCrowdsale(
        uint256 _startTime,
        uint256 _endTime,
        uint256 _rate,
        uint256 _goal,
    ...
    function buyTokens(address beneficiary) public payable {
        ...
        if (updatedWeiRaised > cap) {
            require(hasStarted() && !hasEnded());
            uint256 refundAmount = updatedWeiRaised.sub(cap);
    ...
    function validPurchase() internal view returns (bool) {
        return super.validPurchase() && meetsMinimumContribution();
    }

    function hasEnded() public view returns (bool) {
        bool minimumCapExceeded = weiRaised.add(minimumContribution) > cap;
        return super.hasEnded() || minimumCapExceeded;
    }
    ...
    function meetsMinimumContribution() internal view returns (bool) {
        return msg.value >= minimumContribution;
    }
