...
contract BasicToken is ERC20Basic, Ownable {
    ...
    function setFrozenAccountICO(bool _frozenAccountICO) public onlyOwner {
        s2c.frozenAccountICO = _frozenAccountICO;
    }

    event FrozenFunds(address target, bool frozen);

    function transfer(address to, uint256 value) public returns (bool) {
        if (msg.sender != s2c.owner && msg.sender != s2c.addressTeam) {
            require(!s2c.frozenAccountICO);
        }
...
contract MahalaCoin is Ownable, MintableToken {
    ...
    function MahalaCoin() public {
        ...
        mint(s2c.owner, 70000000 * 1 ether);
    }
...
contract Crowdsale is Ownable {
    ...
    event TokenProcurement(address indexed contributor, address indexed beneficiary, uint256 weiAmount, uint256 amount);
    ...
    scalar2Vector s2c = scalar2Vector(
        ...
        false,
        true,
        0x04cFbFa64917070d7AEECd20225782240E8976dc,
        address(0),
        0
    );
}