...
contract MobaBase {
    ...
    function transferToOwner() onlyOwner msgSendFilter public {
        uint256 totalBalance = address(this).balance;
        owner.transfer(totalBalance);
        emit transferToOwnerEvent(totalBalance);
    }
    
    function updateLock(bool newLockStatus) onlyOwner public {
        require(isLock != newLockStatus, "updateLock new status == old status");
...
contract BRPerSellControl is MobaBase {
    ...
    function updateInteface(address inviteData, address perSellData) onlyOwner msgSendFilter public {
        mInviteAddr = IBRInviteData(inviteData);
        mPerSellData = IBRPerSellData(perSellData);
    ...
    function buyPerSell(uint16 perSellId, bytes32 name) notLock msgSendFilter payable public {
        uint16 id;
        uint256 price;
        bool isOver;
        ...
        if (inviteAddr != address(0)) {
            uint256 giveToEth = msg.value * 10 / 100;
