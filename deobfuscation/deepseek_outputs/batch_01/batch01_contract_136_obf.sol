...
contract Vault is Ownable, VaultInterface {
    ...
    function deposit(address token, uint amount) external payable {
        require(token == state.ETH || msg.value == 0);
        uint value = amount;
    ...
    struct State {
        address latestSpender;
        address ETH;
        address owner;
    }
    
    State state = State(address(0), 0x0, address(0));
}
```