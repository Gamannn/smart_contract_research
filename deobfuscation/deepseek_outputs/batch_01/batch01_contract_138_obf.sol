...
contract Owned {
    ...
    event OwnershipAPITransferred(address indexed previousOwnerAPI, address indexed newOwnerAPI);
...
contract KaasyToken is ERC20Interface, Pausable, SafeMath {
    ...
    uint256 public bonusEnd20;
    uint256 public bonusEnd10;
    uint256 public bonusEnd05;
    uint256 public endDate;
    uint256 public tradingDate;
    ...
    bool public isOwnBlockchainLaunched;
    uint256 public momentOwnBlockchainLaunched;
    ...
    uint8 public teamWOVestingPercentage = 5;
    ...
    constructor() public {
        ...
        startDate = 1535760000;      // Sep 1, 2018
        bonusEnd20 = 1536969600;     // Sep 15, 2018
        bonusEnd10 = 1538179200;     // Sep 30, 2018
        bonusEnd05 = 1539388800;     // Oct 15, 2018
        endDate = 1542240000;        // Nov 15, 2018
        tradingDate = 1543536000;    // Nov 30, 2018
        
        minAmountETH = 1 ether / 10;     // 0.1 ETH
        maxAmountETH = 1 ether * 5000;   // 5000 ETH
        
        exchangeRate = 25000; // 1 ETH = 25000 EUR cents
        
        // Team allocation (15% of total supply)
        uint256 teamAmount = maxSupply * 150 / 1000;
        
        // Team vesting portion (95% of team allocation)
        ...
        // Team non-vesting portion (5% of team allocation)
        ...
        _totalSupply = maxSupply * 40 / 100; // Initial supply (40% of total)
    }
    ...
    function() public payable whenNotPaused {
        if(now > endDate && !isMintingFinished) {
            ...
            // Transfer 15% of ETH to ownerAPI
    ...
    function BurnTokensAndSetAmountForNewBlockchain(address exOwner) public onlyOwnerOrOwnerAPI {
        require(isOwnBlockchainLaunched);
    ...
    function SetNewBlockchainEnabled() public onlyOwner {
        require(isMintingFinished && !isOwnBlockchainLaunched);
    ...
    function getAmountToIssue(uint256 ethAmount) public view returns(uint256) {
        ...
        uint256 ret = euroAmount / 10; // Base rate: 1 EUR = 10 tokens
        
        if(now < bonusEnd20) {
            ret = euroAmount * 12 / 100; // 20% bonus
        } else if(now < bonusEnd10) {
            ret = euroAmount * 11 / 100; // 10% bonus
        } else if(now < bonusEnd05) {
            ret = euroAmount * 105 / 1000; // 5% bonus
        }
        
        // Volume bonuses
        if(euroAmount >= 50000 * 100) { // >= 50,000 EUR
            ret = ret * 13 / 10; // 30% bonus
        } else if(euroAmount >= 10000 * 100) { // >= 10,000 EUR
            ret = ret * 12 / 10; // 20% bonus
        }
        
        return ret * (10 ** uint256(decimals));
    }

    function exchangeEthToEur(uint256 ethAmount) internal view returns(uint256) {
        return safeDiv(safeMul(ethAmount, exchangeRate), 1 ether);
    }

    function exchangeEurToEth(uint256 eurAmount) internal view returns(uint256) {
        return safeDiv(safeMul(safeDiv(safeMul(eurAmount, 1 ether), exchangeRate), 1 ether), 1 ether);
    }
    ...
    function depositsOf(address depositer) public constant returns (uint) {
        return ethDeposits[depositer];
    }

    function burnedBalanceOf(address exOwner) public constant returns (uint) {
        return burnedBalances[exOwner];
    }

    function transfer(address to, uint tokens) public whenNotPaused returns (bool) {
        if(now > endDate && !isMintingFinished) {
            finishMinting();
        }
    ...
    function approveAndCall(address spender, uint tokens, bytes data) public whenNotPaused returns (bool) {
        allowed[msg.sender][spender] = tokens;
        emit Approval(msg.sender, spender, tokens);
    ...
    function transferAllERC20Token(address tokenAddress, uint tokens) public onlyOwnerOrOwnerAPI returns (bool) {
        return ERC20Interface(tokenAddress).transfer(owner, tokens);
    }

    function transferAnyERC20Token(address tokenAddress) public onlyOwnerOrOwnerAPI returns (bool) {
        return ERC20Interface(tokenAddress).transfer(owner, ERC20Interface(tokenAddress).balanceOf(this));
    }

    function updateExchangeRate(uint newEthEurRate) public onlyOwnerOrOwnerAPI returns (bool) {
    ...
    function updateEndDate(uint256 newDate) public onlyOwnerOrOwnerAPI returns (bool) {
    ...
    function updateTokenNameSymbolAddress(string newTokenName, string newSymbol, address newContractAddress) public whenPaused onlyOwnerOrOwnerAPI returns (bool) {
        name = newTokenName;
        symbol = newSymbol;
