...
contract ARXCrowdsale is owned, safeMath {
    ...
    mapping(address => uint256) public usersARXfundValue;
    ...
    function SetupCrowdsale(uint256 _fundingStartBlock, uint256 _fundingEndBlock) public onlyOwner returns (bytes32 response) {
        if ((msg.sender == admin) && (!isCrowdSaleSetup) && (!(beneficiaryWallet > 0))) {
    ...
    function checkGoalReached() public onlyOwner {
        ...
        if ((amountRaisedInWei < fundingMinCapInWei) && 
            (block.number <= fundingEndBlock && block.number >= fundingStartBlock)) {
