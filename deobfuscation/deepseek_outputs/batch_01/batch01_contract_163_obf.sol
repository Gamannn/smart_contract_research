...
contract EPXCrowdsale is owned, safeMath {
    ...
    mapping(address => uint256) public usersEPXfundValue;
    ...
    function SetupCrowdsale(uint256 _fundingStartBlock, uint256 _fundingEndBlock) public onlyOwner returns (bytes32 response) {
        if ((msg.sender == deployer) && (!isCrowdSaleSetup) && (!(beneficiaryWallet > 0))) {
    ...
    function () public payable {
        require(!(msg.value == 0) && (msg.data.length == 0) && (block.number <= fundingEndBlock) && (block.number >= fundingStartBlock) && (tokensRemaining > 0));
    ...
    function refund() public {
        require((amountRaisedInWei < fundingMinCapInWei) && (isCrowdSaleClosed) && (block.number > fundingEndBlock) && (usersEPXfundValue[msg.sender] > 0));
