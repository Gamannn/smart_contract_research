...
contract Ethernext is SafeMath {
    ...
    constructor(
        ...
        exchangeInfo.admin = admin_;
        exchangeInfo.feeAccount = feeAccount_;
        exchangeInfo.accountLevelsAddr = accountLevelsAddr_;
        exchangeInfo.feeMake = feeMake_;
        exchangeInfo.feeTake = feeTake_;
        exchangeInfo.feeRebate = feeRebate_;
    }
    ...
    function changeAdmin(address admin_) public {
        if (msg.sender != exchangeInfo.admin) revert();
        exchangeInfo.admin = admin_;
    }
    ...
    function changeFeeMake(uint feeMake_) public {
        if (msg.sender != exchangeInfo.admin) revert();
        if (feeMake_ > exchangeInfo.feeMake) revert();
        exchangeInfo.feeMake = feeMake_;
    }
    
    function changeFeeTake(uint feeTake_) public {
        if (msg.sender != exchangeInfo.admin) revert();
        if (feeTake_ > exchangeInfo.feeTake || feeTake_ < exchangeInfo.feeRebate) revert();
        exchangeInfo.feeTake = feeTake_;
    }
    
    function changeFeeRebate(uint feeRebate_) public {
        if (msg.sender != exchangeInfo.admin) revert();
        if (feeRebate_ < exchangeInfo.feeRebate || feeRebate_ > exchangeInfo.feeTake) revert();
        exchangeInfo.feeRebate = feeRebate_;
    }
    ...
    function tradeBalances(
        ...
        uint feeMakeXfer = safeMul(amount, exchangeInfo.feeMake) / (1 ether);
        uint feeTakeXfer = safeMul(amount, exchangeInfo.feeTake) / (1 ether);
        uint feeRebateXfer = 0;
        
        if (exchangeInfo.accountLevelsAddr != 0x0) {
            uint accountLevel = AccountLevels(exchangeInfo.accountLevelsAddr).accountLevel(user);
            if (accountLevel == 1) {
                feeRebateXfer = safeMul(amount, exchangeInfo.feeRebate) / (1 ether);
            }
            if (accountLevel == 2) {
        ...
        tokens[tokenGet][exchangeInfo.feeAccount] = safeAdd(tokens[tokenGet][exchangeInfo.feeAccount], safeSub(safeAdd(feeMakeXfer, feeTakeXfer), feeRebateXfer));
        
        tokens[tokenGive][user] = safeSub(tokens[tokenGive][user], amountGive * amount / amountGet);
        tokens[tokenGive][msg.sender] = safeAdd(tokens[tokenGive][msg.sender], amountGive * amount / amountGet);
    }
