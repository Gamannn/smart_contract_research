...
contract Token {
    ...
    function balanceOf(address owner) constant public returns (uint256 balance) {}
    function transfer(address to, uint256 value) public returns (bool success) {}
    function transferFrom(address from, address to, uint256 value) public returns (bool success) {}
    function approve(address spender, uint256 value) public returns (bool success) {}
    function allowance(address owner, address spender) constant public returns (uint256 remaining) {}
...
contract ReserveToken is StandardToken, SafeMath {
    function ReserveToken() public {
        dex.minter = msg.sender;
    }
    
    function create(address account, uint amount) public {
        if (msg.sender != dex.minter) revert();
        balances[account] = safeAdd(balances[account], amount);
        dex.totalSupply = safeAdd(dex.totalSupply, amount);
    }
    ...
    function destroy(address account, uint amount) public {
        ...
        dex.totalSupply = safeSub(dex.totalSupply, amount);
    }
}
...
contract PolarisDEX is SafeMath {
    ...
    function PolarisDEX(address admin_, address feeAccount_, address accountLevelsAddr_, uint feeMake_, uint feeTake_, uint feeRebate_) public {
        dex.admin = admin_;
        dex.feeAccount = feeAccount_;
        dex.accountLevelsAddr = accountLevelsAddr_;
        dex.feeMake = feeMake_;
        dex.feeTake = feeTake_;
        dex.feeRebate = feeRebate_;
    }
    ...
    function changeAdmin(address admin_) public {
        if (msg.sender != dex.admin) revert();
        dex.admin = admin_;
    }
    ...
    function changeFeeMake(uint feeMake_) public {
        if (msg.sender != dex.admin) revert();
        if (feeMake_ > dex.feeMake) revert();
        dex.feeMake = feeMake_;
    }
    
    function changeFeeTake(uint feeTake_) public {
        if (msg.sender != dex.admin) revert();
        if (feeTake_ > dex.feeTake || feeTake_ < dex.feeRebate) revert();
        dex.feeTake = feeTake_;
    }
    
    function changeFeeRebate(uint feeRebate_) public {
        if (msg.sender != dex.admin) revert();
        if (feeRebate_ < dex.feeRebate || feeRebate_ > dex.feeTake) revert();
        dex.feeRebate = feeRebate_;
    }
    ...
    function tradeBalances(address tokenGet, uint amountGet, address tokenGive, uint amountGive, address user, uint amount) private {
        uint feeMakeXfer = safeMul(amount, dex.feeMake) / (1 ether);
        uint feeTakeXfer = safeMul(amount, dex.feeTake) / (1 ether);
        uint feeRebateXfer = 0;
        
        if (dex.accountLevelsAddr != 0x0) {
            uint accountLevel = AccountLevels(dex.accountLevelsAddr).accountLevel(user);
            if (accountLevel == 1) feeRebateXfer = safeMul(amount, dex.feeRebate) / (1 ether);
            if (accountLevel == 2) feeRebateXfer = feeTakeXfer;
        }
        
        tokens[tokenGet][msg.sender] = safeSub(tokens[tokenGet][msg.sender], safeAdd(amount, feeTakeXfer));
        tokens[tokenGet][user] = safeAdd(tokens[tokenGet][user], safeSub(safeAdd(amount, feeRebateXfer), feeMakeXfer));
        tokens[tokenGet][dex.feeAccount] = safeAdd(tokens[tokenGet][dex.feeAccount], safeSub(safeAdd(feeMakeXfer, feeTakeXfer), feeRebateXfer));
    ...
    DexData dex = DexData(0, 0, 0, address(0), address(0), address(0), address(0), 0, "", 0);
}
```