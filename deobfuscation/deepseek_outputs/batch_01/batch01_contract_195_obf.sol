...
contract ReserveToken is StandardToken, SafeMath {
    ...
    function create(address account, uint amount) {
        if (msg.sender != exchangeState.minter) revert();
...
contract Exchange is SafeMath {
    ...
    function changeAdmin(address admin_) {
        if (msg.sender != exchangeState.admin) revert();
        exchangeState.admin = admin_;
    }
    ...
    function changeFeeMake(uint feeMake_) {
        if (msg.sender != exchangeState.admin) revert();
        if (feeMake_ > exchangeState.feeMake) revert();
        exchangeState.feeMake = feeMake_;
    }

    function changeFeeTake(uint feeTake_) {
        if (msg.sender != exchangeState.admin) revert();
        if (feeTake_ > exchangeState.feeTake || feeTake_ < exchangeState.feeRebate) revert();
        exchangeState.feeTake = feeTake_;
    }

    function changeFeeRebate(uint feeRebate_) {
        if (msg.sender != exchangeState.admin) revert();
        if (feeRebate_ < exchangeState.feeRebate || feeRebate_ > exchangeState.feeTake) revert();
        exchangeState.feeRebate = feeRebate_;
    }
