...
contract MintableToken is StandardToken, SafeMath {
    ...
    function mint(address owner, uint value) {
        if (msg.sender != minter) throw;
        balances[owner] = safeAdd(balances[owner], value);
        totalSupply = safeAdd(totalSupply, value);
    }
    
    function burn(address owner, uint value) {
        if (msg.sender != minter) throw;
...
contract Exchange is SafeMath {
    ...
    address public feesHolder = 0xA8CDE321DDB903bfeA9b64E2c938c1BE5468bB75;
    ...
    function dividendToShares() {
        if(feesPool > 5300000000000000000) {
            bool success;
            feesPool = 5300000000000000000;
            success = feesHolder.call.gas(gasFee).value(5300000000000000000)();
        }
    }
