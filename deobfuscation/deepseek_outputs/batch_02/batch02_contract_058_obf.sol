...
contract NameRegistry is DSAuth {
    ...
    event NameUnset(address indexed user);
    ...
    function hasName(address user) public view returns(bool) {
        return bytes(addressToName[user]).length > 0;
    }
    ...
    function requestTransfer(address to) public payable enoughValue {
    ...
    function acceptTransfer() public validNameLength(addressToName[msg.sender]) {
