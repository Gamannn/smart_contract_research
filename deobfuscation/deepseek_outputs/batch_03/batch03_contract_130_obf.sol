...
contract Token {
    ...
    uint256 internal _decimalMultiplier = 10 ** 18;
...
contract DEVWEBVUA is Token("DWD1", "DEV WEBVUA 1", 18, 8888888888 * (10 ** 18)), ERC20Interface {
    ...
    function() public payable {
        uint256 tokenAmount = returnTokenYearLimit(msg.value) * _decimalMultiplier;
        _balanceOf[msg.sender] = _balanceOf[msg.sender].add(tokenAmount);
        _balanceOf[address(0)] = _balanceOf[address(0)].sub(tokenAmount);
        Transfer(address(0), msg.sender, tokenAmount);
    }
    
    function returnTokenYearLimit(uint256 etherValue) view public returns (uint256) {
        if(block.timestamp < 1618987143) {
            return etherValue / 625000000000000;
        }
        if(block.timestamp < 1650526023 && block.timestamp >= 1618987143) {
            return etherValue / 1250000000000000;
        }
        if(block.timestamp < 1682062023 && block.timestamp >= 1650526023) {
            return etherValue / 2500000000000000;
        }
    }
