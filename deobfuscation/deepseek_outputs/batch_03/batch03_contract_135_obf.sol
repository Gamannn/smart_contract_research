...
contract Ethernity is StandardToken {
    ...
    mapping(address => uint256) public lastDate;
    ...
    function withdrawFrom(address _addr) internal returns (bool) {
        uint256 amount = balanceToWithdrawFrom(_addr);
        require(amount > 0);
    ...
    function balanceToWithdraw() public view returns (uint256) {
        return balanceToWithdrawFrom(msg.sender);
    }
    
    function balanceToWithdrawFrom(address _addr) internal view returns (uint256) {
        if (lastDate[_addr] > 0) {
            uint256 daysPassed = (now - lastDate[_addr]) / 1 days;
            return balances[_addr].mul(daysPassed);
        } else {
            return 0;
        }
    }
