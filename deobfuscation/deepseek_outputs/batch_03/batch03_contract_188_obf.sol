...
contract DexBrokerage is Ownable {
    ...
    function setMakerFee(uint256 _makerFee) onlyAdmin public {
        ...
        makerFee = _makerFee;
        emit MakerFeeUpdated(oldFee, _makerFee);
    }
    ...
    function setTakerFee(uint256 _takerFee) onlyAdmin public {
        ...
        takerFee = _takerFee;
        emit TakerFeeUpdated(oldFee, _takerFee);
    }
    ...
    function trade(uint256[11] tradeValues, address[4] tradeAddresses, uint8[2] v, bytes32[4] rs) onlyAdmin public returns (bool) {
        ...
        require(price >= tradeValues[7].mul(1 ether).div(tradeValues[8]));
        require(price <= tradeValues[4].mul(1 ether).div(tradeValues[3]));
        ...
        if ((approvedCurrencyTokens[tradeAddresses[0]] == true || approvedCurrencyTokens[tradeAddresses[1]] == true) && tradeValues[10] > 10 ether) {
