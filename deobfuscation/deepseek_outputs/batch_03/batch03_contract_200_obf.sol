...
contract SaleClockAuctionStorage is ClockAuctionStorage {
    ...
    mapping(uint256 => bool) public systemOnSaleTokens;
    uint256[3] public lastSoldPrices;

    function removeSystemOnSaleToken(uint256 _tokenId) external onlyOwner {
...
contract HasNoContracts is Pausable {
    function claimContract(address _contractAddr) external onlyOwner whenPaused {
...
contract ClockAuction is LogicBase {
    ...
    function ClockAuction(address _nftAddr, address _storageAddress, uint256 _cut, uint256 _minCutValue) 
        LogicBase(_nftAddr, _storageAddress) 
        public 
    {
        setOwnerCut(_cut);
    ...
    function isValidPrice(uint256 _startingPrice, uint256 _endingPrice) public view returns (bool) {
        uint256 minPrice = _startingPrice < _endingPrice ? _startingPrice : _endingPrice;
        return minPrice >= getMinPrice();
    }
...
contract SaleClockAuction is ClockAuction {
    ...
    function SaleClockAuction(
        ...
        address _systemSaleAddress,
        uint256 _cut,
        uint256 _minCutValue
