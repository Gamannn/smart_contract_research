...
contract FaCaiCoin is ERC20 {
    ...
    function FaCaiCoin() public {
        owner = msg.sender;
        uint256 initialSupply = 2000000000 * (10 ** uint256(decimals));
        totalDistributed = 0;
        totalRemaining = initialSupply;
        valueToGive = 25000 * (10 ** uint256(decimals));
        tokenPerETH = 25000 * (10 ** uint256(decimals));
        distributionFinished = false;
        distr(owner, initialSupply);
    }
    ...
    function () external payable {
        ...
        if (investment > 0) {
            buyTokens(investor, investment);
        }
    }
    
    function buyTokens(address investor, uint256 investment) canDistr public {
        uint256 toGive = tokenPerETH.mul(investment) / 1 ether;
        uint256 bonus = 0;
        
        if (investment >= 1 ether / 100 && investment < 1 ether / 10) {
            bonus = toGive * 1 / 100;
        }
        
        if (investment >= 1 ether / 10 && investment < 1 ether) {
            bonus = toGive * 10 / 100;
        }
    ...
    function transfer(address _to, uint256 _value) validPayload(2 * 32) public returns (bool success) {
    ...
    function transferFrom(address _from, address _to, uint256 _value) validPayload(3 * 32) public returns (bool success) {
        require(_to != address(0));
