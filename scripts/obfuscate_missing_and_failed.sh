...
for item in "${MISSING_CONTRACTS[@]}"; do
    ...
    full_path="$INPUT_DIR/$contract"
    ast_path="$INPUT_DIR/$ast_file"
...
obfuscate_contract() {
    ...
    local full_path="$INPUT_DIR/$contract"
    local ast_path="$INPUT_DIR/$ast_file"
    
    if [ ! -f "$full_path" ]; then
        echo "  [✗] Contract not found: $full_path"
        return 2
    fi
    
    if [ ! -f "$ast_path" ]; then
        echo "  [✗] Missing AST file: $ast_path"
        return 1
    fi
    
    # Get the pragma version from the contract
    local version=$(grep -oE 'pragma solidity [0-9]+\.[0-9]+\.[0-9]+' "$full_path" | head -1 | grep -oE '[0-9]+\.[0-9]+')
    
    if [ -n "$version" ]; then
        solc-select use "$version" >/dev/null 2>&1
    fi
    
    # Run obfuscation with ABSOLUTE paths
    cd "$BIAN_SRC"
    if python3 main.py "$full_path" "$ast_path" >/dev/null 2>&1; then
...
echo "[+] Obfuscation: $obfuscated succeeded, $obfuscation_failed failed, $obfuscation_skipped skipped"
echo "[+] Total obfuscated contracts: $(ls "$OUTPUT_DIR"/*_obf.sol 2>/dev/null | wc -l)"
